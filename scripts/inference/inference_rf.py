...
def find_config_file(model_path):
    ...
    # 模型文件名: rf_onehot_model.pkl -> 配置: rf_onehot_config.json
    # 去掉 "_model" 后缀和扩展名
...
def load_model(model_path):
    ...
    config_path = find_config_file(Path(model_path))
    if config_path:
...
def main():
    parser = argparse.ArgumentParser(description="RF模型推理")
    ...
    if config:
        config_encoding = config.get('encoding', 'esm2')
        print(f"  算法: {config.get('algorithm', 'RF')}")
        print(f"  编码: {config_encoding}")
    else:
        config_encoding = 'esm2'
        print("  警告: 未找到配置文件，使用默认编码")
    
    # 优先使用命令行参数
    encoding = args.encoding or config_encoding
