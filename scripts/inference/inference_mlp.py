...
def predict(sequence, model, scaler, encoder):
    """单条预测"""
...
def main():
    parser = argparse.ArgumentParser(description="MLP模型推理")
    ...
    if config:
        ...
        print(f"  算法: {config.get('algorithm', 'MLP')}")
        print(f"  编码: {encoding}")
    else:
        encoding = 'esm2'
    ...
    if args.sequence:
        result = predict(args.sequence, model, scaler, encoder)
        ...
        for i, (header, seq) in enumerate(zip(headers, sequences)):
            result = predict(seq, model, scaler, encoder)
